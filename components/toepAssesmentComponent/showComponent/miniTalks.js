...
export default function miniTalks() {
  ...
  const columns = [
    {
      ...
      sortable: false,
      renderCell: (params) => {
        const audioUrl = params.row.files; // Ganti dengan atribut yang sesuai dengan URL audio dalam data Anda
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(
        process.env.NEXT_PUBLIC_API_URL + `/soal-toep?part_soal=MINI TALKS`,
        {
          headers: {
            Authorization: `Bearer ${session.user.token}`,
          },
