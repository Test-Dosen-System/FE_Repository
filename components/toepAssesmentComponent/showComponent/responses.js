...
export default function responses() {
  ...
  const columns = [
    {
      ...
      sortable: false,
      renderCell: (params) => {
        const audioUrl = params.row.files;
        return <ReactAudioPlayer src={audioUrl} controls style={customStyle} />;
      },
    },
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(
        process.env.NEXT_PUBLIC_API_URL + `/soal-toep?part_soal=RESPONSES`,
        {
          headers: {
            Authorization: `Bearer ${session.user.token}`,
          },
  ...
  const updateHandler = async (id) => {
    try {
      ...
      const response = await axios.put(
        process.env.NEXT_PUBLIC_API_URL + `/soal-toep/update-audio/${id}`,
        detailData,
        {
          headers: {
            Authorization: `Bearer ${session.user.token}`,
              ...
              <Grid item xs={12}>
                ...
                >
                  <input
                    ...
                    type="file"
                    onChange={(e) =>
                      setDetailData({ ...detailData, files: e.target.files[0] })
                    }
                  />
                </IconButton>
              </Grid>
